...
moment.locale('pl');
  ...
  bot.on("ready", e => {
	setInterval (function (){
	  bot.channels.get("515916263613005837").edit({ name: `➤ Użytkownicy: ${bot.guilds.get("454946768723902476").memberCount}`});    
	  
	  bot.channels.get("515916480865370128").edit({ name: `➤ Online: ${bot.guilds.get("454946768723902476").members.filter(m => m.presence.status === 'online').size}`});
		
	  bot.channels.get("515916718556577793").edit({ name: `➤ Dzień: ${moment(this.date).format('DD.MM.YYYY')}`});
		
	  bot.channels.get("515916401328783360").edit({ name: `➤ Administracja: ${bot.guilds.get("454946768723902476").roles.get("515933131807784968").members.size}`}); 
